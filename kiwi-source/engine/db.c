...
#include <pthread.h>                                        // for ptheards.
#include <stdbool.h>                                        // for the signal.

DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
    DB* self = calloc(1, sizeof(DB));

    pthread_cond_init(&reader_cond, NULL);			        // declarations of conditions and mutexes.					
    pthread_cond_init(&writer_cond, NULL);
    pthread_mutex_init(&lock, NULL);
    readers_count = 0;                                      // to count readers.
    signal = false;                                         // variable for readwrite.
...
{
    int add_value;                                          // the value that will be returned from db_add.                           
    pthread_mutex_lock(&lock);                              // we want one thread to write so we lock it in that time.
    while(readers_count >=1){                               // while there is a thread reading we stop and wait for it's signal so we can continue.
        pthread_cond_wait(&reader_cond,&lock);
    }
    signal = true;                                          // we set the flag true because one thread writes.
    ...
    add_value = memtable_add(self->memtable, key, value);   // setting add_value to the return of memtable_add(write).
    signal = false;                                         // we inform that writing is over.
    pthread_cond_signal(&writer_cond);                      // waking up a reader so it can start reading after write ended.
    pthread_mutex_unlock(&lock);                            // we unlock
...
{
    int get_value;                                          // the value that will be returned from db_get.  
    pthread_mutex_lock(&lock);                              // we want one thread to write so we lock it in that time.
    while(signal==true){                                    // while a thread is writing we stop and wait for its signal.
        pthread_cond_wait(&writer_cond,&lock);              // the operation finished.

    }
    readers_count++;                                        // plus one thread.

    pthread_mutex_unlock(&lock);                            
    if (memtable_get(self->memtable->list, key, value) != 1){ // because we are out of the critical area we can use multiply read threads at the same time.
        get_value = sst_get(self->sst,key,value);

    }else{
        get_value = 1;
    }

    pthread_mutex_lock(&lock);                              // we lock here.
    readers_count--;                                        // readers_count -1 because that thread made a read.

    if (readers_count == 0){
        pthread_cond_signal(&reader_cond);                  // we have zero reading threads so we wake up the writing thread.
    }
